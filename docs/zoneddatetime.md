...
If `zonedDateTimeLike` is a string, then it will be attempted to be converted into a `Temporal.ZonedDateTime`, `Temporal.PlainDate` (if no time is given in the string), `Temporal.PlainDateTime`, or `Temporal.PlainTime`, in order of priority.
...
date = Temporal.PlainDate.from('2015-05-31');
